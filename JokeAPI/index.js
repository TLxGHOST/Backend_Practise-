...
app.get("/filter",(req,res)=>{
   ...
   let joke=jokes.filter((j)=>j.jokeType===type)
   res.send(joke);
})
...
app.put("/jokes/:id",(req,res)=>{
  ...
  jokes[parseInt(req.params.id)]=newJ;
  res.json(newJ);
})

app.patch("/jokes/:id",(req,res)=>{
  let id=parseInt(req.params.id);
  const backup=jokes[id-1];
  const NewJoke={
    id:backup.id,
    jokeText:(req.body.text)?req.body.text:backup.jokeText,
    jokeType:(req.body.type)?req.body.type:backup.jokeType,
  }
  jokes[id-1]=NewJoke;
  res.json(NewJoke);
})
